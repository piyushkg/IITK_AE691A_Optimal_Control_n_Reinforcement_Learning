...
aref = omega*omega*AU;
x0 = 0.0; y0 = -1.5e11/AU;
vx0 = 30000.0/vref; vy0 = 0.0;
xT = 0.0; yT = 4.15e11/AU;
vxT = -18000.0/vref; vyT = 0.0;
mu_con = G*Msun/(omega*omega*AU*AU*AU);
u = [ax;ay];
T = 473.0*24.0*3600.0*omega;
% Model equations
...
Xk = MX.sym('X0', 4);
w = {w{:}, Xk};
lbw = [lbw; x0;y0;vx0;vy0];
ubw = [ubw; x0;y0;vx0;vy0];
w0 = [w0; x0;y0;vx0;vy0];
...
for k=0:N-1
    ...
    w = {w{:}, Xk};
    if(k==N-1)
        lbw = [lbw;xT;yT;vxT;vyT];
        ubw = [ubw;xT;yT;vxT;vyT];
        w0 = [w0; xT;yT;vxT;vyT];
    else
        lbw = [lbw;-inf;-inf;-inf;-inf];
        ubw = [ubw;  inf;inf;inf;inf];
...
% plot(tgrid(1:100), u1_opt', '--')
% plot(tgrid(1:100), u2_opt', '-')
% xlabel('t')
% legend('ax','ay')
% grid on
